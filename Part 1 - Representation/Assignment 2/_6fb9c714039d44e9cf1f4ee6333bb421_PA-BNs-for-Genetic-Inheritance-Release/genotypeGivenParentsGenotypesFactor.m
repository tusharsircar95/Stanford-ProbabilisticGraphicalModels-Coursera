...
possibleAlleles = (numAlleles*(numAlleles-1)/2) + numAlleles;
genotypeFactor.card = [possibleAlleles,possibleAlleles,possibleAlleles];
genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.
assignments = IndexToAssignment(1:prod(genotypeFactor.card),genotypeFactor.card);
for i = 1:size(assignments,1)
	assignment = assignments(i,:);
	p1 = genotypesToAlleles(assignment(1),:);
	p2 = genotypesToAlleles(assignment(2),:);
	p3 = genotypesToAlleles(assignment(3),:);
	
	probability = 0;
	if p1(1) == p1(2)
		probability = (sum(ismember(p2,p1(1)))/2)*(sum(ismember(p3,p1(2)))/2);
	end
	
	if p1(1) != p1(2)
		probability = (sum(ismember(p2,p1(1)))/2)*(sum(ismember(p3,p1(2)))/2) + (sum(ismember(p3,p1(1)))/2)*(sum(ismember(p2,p1(2)))/2);
	end
	
	genotypeFactor.val(i) = probability;
